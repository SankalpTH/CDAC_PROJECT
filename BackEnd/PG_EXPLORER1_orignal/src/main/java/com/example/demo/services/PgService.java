...
public class PgService {
    ...
    public String addPg(int oid, DummyPgImages dpi) throws Exception {
        if (dpi == null) {
            throw new Exception("Invalid data provided");
        }

        System.out.println("Images : " + (dpi.getImage1() != null ? dpi.getImage1().getBytes().length : "No Image"));
        
        Owner owner = orepo.findById(oid).orElseThrow(() -> new Exception("Owner does not exist"));
        Area area = arepo.findById(dpi.getAreaId()).orElseThrow(() -> new Exception("Area not found: " + dpi.getAreaId()));
 ...
 // Update existing PG with new details and images
    ...
    public List<Pg> getPg(String aname,int cityId, boolean wifi, boolean ac, boolean laundry) {
        return prepo.findPGsByFilters(aname,cityId, wifi, ac, laundry);
    }

    public List<Pg> getPgByOwner(int oid) {
        return prepo.findPGByOwner(oid);
    }
    public Optional<Pg> getPgOnClick(int pid) {
        return prepo.findById(pid);
    }
